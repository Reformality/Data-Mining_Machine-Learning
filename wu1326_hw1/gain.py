...
class UncertaintyScorer(ABC):
	...
	def split_on_best(self, X, y, exclude=set()):
		...
		for j in range(num_features):
			...
			## >>> YOUR CODE HERE >>>
			tempGain = self.compute_gain(X, y, j)
			if tempGain > best_gain:
				best_gain = tempGain
				best_feature = j
		...
		## >>> YOUR CODE HERE >>>
		attrValueSet = set(X[:,best_feature])
		for attrValue in attrValueSet:
			X_sub, y_sub = self.subset_data(X, y, best_feature, attrValue)
			subsets[attrValue] = (X_sub, y_sub)
...
class InformationGain(UncertaintyScorer):
	...
	def score(self, y):
		...
		## >>> YOUR CODE HERE >>>
		entropy = 0
		for labelKey, probValue in probs.items():
			entropy = entropy - probValue * np.log2(probValue)
	...
	def compute_gain(self, X, y, j):
		...
		"""
		## >>> YOUR CODE HERE >>>
		entropy = self.score(y) ## Entropy(S)
		information_gain = 0
		attrValueSet = set(X[:,j])
		for attrValue in attrValueSet:
			X_sub, y_sub = self.subset_data(X, y, j, attrValue)
			information_gain = information_gain + (np.shape(X)[0] / np.shape(X_sub)[0]) * self.score(y_sub)
