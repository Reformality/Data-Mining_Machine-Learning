...
class KNN():
    ...
    def get_distances_to_other_samples(self, x: np.ndarray) -> np.ndarray:
        ...
        for i in range(np.shape(self.X_train)[0]):
            distVal = self.euclidean_distance(self.X_train[i,:], x)
            dist = np.append(dist, distVal)
    ...
    def vote(self, k_nearest: np.ndarray) -> int:
        ...
        # >> YOUR CODE HERE
        kCounts = np.bincount(k_nearest)
        y_pred = np.argmax(kCounts)
        # END OF YOUR CODE <<
    ...
    def get_k_nearest_neighbors(self, dist: np.ndarray) -> np.ndarray:
        ...
        k_nearest_full = self.y_train[sortedIndex]
        k_nearest = k_nearest_full[0:self.k]
        # END OF YOUR CODE <<
...
def evaluate_KNN() -> None:
    ...
    # and validation data could not be seenï¼
    knn = KNN(k=5)
